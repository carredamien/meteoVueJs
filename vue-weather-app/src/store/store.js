...
const store = new Vuex.Store({
  state: {
    ...
    apiKey: "2b1d940e98362d0725b60eb9bcaf3f88",   // Create Api Key from https://openweathermap.org
    ...
    async fetchWeatherData({ commit, state }, search) {
      try {
        ...
        const response = await axios.get(
          `${state.apiBase}weather?q=${search}&units=metric&APPID=${state.apiKey}&lang=fr`
        );
